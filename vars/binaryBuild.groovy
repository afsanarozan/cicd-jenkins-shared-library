...
def buildBinary(Map args) {
    sh """
        ...
        git config --global url."https://automation-labs:glpat-7BrmVPQpz-Dy9VbuxXbB@gitlab.com".insteadOf "https://gitlab.com"
        go mod verify
        go mod tidy -e 
...
def pushBinary(Map args) {
    ...
    try {
        sh "aws s3 cp --endpoint-url ${args.spaces_url} s3://binary-build/${args.application_name}/${args.environment}/${args.service_name}-latest ./${args.service_name}-${args.dstVersion}"
        sh "aws s3 cp --endpoint-url ${args.spaces_url} ./${args.service_name}-${args.dstVersion} s3://binary-build/${args.application_name}/${args.environment}/"
        sh "rm ${args.service_name}-${args.dstVersion}"
    } catch (e) {
        echo "first time push binary build service to do spaces"
    } finally {
        sh "aws s3 cp ${args.service_name} --endpoint-url ${args.spaces_url} s3://binary-build/${args.application_name}/${args.environment}/${args.service_name}-latest"
    }
}